...
class InviteForm(forms.Form, CleanEmailMixin):
    ...
    type = forms.IntegerField(
        label=_("Type"),
        required=True,
        widget=forms.TextInput(attrs={"type": "type", "size": "30"}), initial="")

    def save(self, email):
        return Invitation.create(email=email, type=type)
...
class InvitationAdminAddForm(forms.ModelForm, CleanEmailMixin):
    ...
    type = forms.IntegerField(
        label=_("Type"),
        required=True,
        widget=forms.TextInput(attrs={"type": "type", "size": "30"}))
